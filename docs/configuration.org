...
=mappings.prefix= applies to every mapping category. All categories
except =global= are registered buffer local (in org files, the agenda
buffer, capture buffers and special edit buffers), while =global=
(=org_agenda= and =org_capture=) is registered globally. If you want a
buffer local prefix like =<LocalLeader>= for the org mappings, the two
global ones would end up on that prefix as well and shadow mappings of
other filetypes. Use =mappings.global_prefix= to give the =global=
category its own prefix. It defaults to =nil=, which falls back to
=mappings.prefix=:
