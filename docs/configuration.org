...
agenda types into single view. Each type in such a view is a foldable
block: the usual fold commands (=zc=, =zo=, =za=, =zM=, =zR=) work on it, and
[[#org_agenda_goto][org_agenda_goto]] toggles the block when pressed on its
header. Available options for each agenda type are explained down below the
example:
...
On the header of a block in a [[#org_agenda_custom_commands][custom agenda command]]
with multiple types, toggle the fold of that block instead.
